...
function HoverDetector({ 
  ...
  useEffect(() => {
    const handleMouseMove = (event: MouseEvent) => {
      ...
      // Calculate approximate grid position based on camera projection
      const direction = raycaster.ray.direction;
      const origin = raycaster.ray.origin;
      
      // Find intersection with y = -0.85 plane (where boxes are positioned)
      const t = (-0.85 - origin.y) / direction.y;
      const intersectionPoint = origin.clone().add(direction.clone().multiplyScalar(t));
      
      // Convert world coordinates to grid coordinates
      const gridX = Math.round((intersectionPoint.x + (gridSize - 1) * spacingX / 2) / spacingX);
      const gridZ = Math.round((intersectionPoint.z + (gridSize - 1) * spacingZ / 2) / spacingZ);
      
      // Check if within grid bounds
      if (gridX >= 0 && gridX < gridSize && gridZ >= 0 && gridZ < gridSize) {
         // Debug log
        onHoverChange([gridX, gridZ]);
      } else {
        onHoverChange(null);
      }
    };
  ...
  }, [camera, gridSize, spacingX, spacingZ, onHoverChange]);
...
export function ChromeGridClient() {
  return (
