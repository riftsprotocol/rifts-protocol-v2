...
use anchor_spl::token_interface::{
    self, burn as interface_burn, mint_to as interface_mint_to, transfer as interface_transfer,
    Burn, Mint, MintTo, TokenAccount, TokenInterface, Transfer,
};
...
declare_id!("BMNSJjxkF9KmDRBJNdiXTWYXqagv5sdmF9JQUSnDgBJY");

// **MEDIUM FIX #6**: Centralize program authority constant
const PROGRAM_AUTHORITY: &str = "4UAWKQ94DXYcUAWw3hddLThq3bn9i3jxCZE3DAnbSN2K";

// **SECURITY FIX**: Second admin authority for critical operations (emergency withdraw)
const ADMIN_AUTHORITY_2: &str = "8xymifw4FrLazZKG3SDouRSLVmMcFBBnF13iCtdEdEd2";
...
pub mod rifts_protocol {
    ...
    pub fn create_rift_with_vanity_pda(
        ...
        // **FIX CRITICAL #31**: Validate Token-2022 extensions to prevent DoS and vault drain
        {
            ...
            if *mint_info.owner == anchor_spl::token::ID {
                ...
                // **FIX HIGH #29**: Reject mints with freeze authority
                require!(
                    mint.freeze_authority == COption::None,
                    ErrorCode::UnsafeUnderlyingMint
                );

                // **FIX HIGH #30**: Reject mints with mint authority (can inflate supply)
                require!(
                    mint.mint_authority == COption::None,
                    ErrorCode::UnsafeUnderlyingMint
                );

                msg!("✅ SPL Token mint validated: no freeze/mint authority");
                ...
                // **FIX CRITICAL #31**: Validate Token-2022 extensions
        ...
        // 1. Calculate space for Token-2022 mint with transfer fee extension
        let mint_space =
        ...
        // 2. Create mint account with sufficient space for extension
        ...
        // Create metadata for the rift token
        let rift_name_str =
            core::str::from_utf8(&rift.name[..name_len as usize]).unwrap_or("Rift Token");
        let symbol = "r".to_string() + &rift_name_str[..rift_name_str.len().min(10)];
        ...
        let create_ix = mpl_token_metadata::instructions::CreateMetadataAccountV3 {
            ...
            mint: ctx.accounts.rift_mint.key(),
            mint_authority: ctx.accounts.rift_mint_authority.key(),
        ...
        let ix_args = mpl_token_metadata::instructions::CreateMetadataAccountV3InstructionArgs {
            data: metadata_data,
            is_mutable: true,
        ...
        msg!(
            "✅ Created metadata for rift token: {} ({})",
            rift_name_str,
            symbol
        );
        ...
        // Determine token program for underlying mint
    ...
    pub fn create_rift(
        ...
        rift.pending_pyth_account = None;
    ...
    pub fn wrap_tokens(ctx: Context<WrapTokens>, amount: u64, min_rift_out: u64) -> Result<()> {
        ...
        let execution_result = (|| -> Result<()> {
            ...
            interface_transfer(transfer_ctx, amount)?;
            ...
            if wrap_fee > 0 && rift.fees_vault != anchor_lang::solana_program::system_program::ID {
                ...
                interface_transfer(fee_transfer_ctx, wrap_fee)?;
    ...
    pub fn unwrap_from_vault(ctx: Context<UnwrapFromVault>, rift_token_amount: u64, min_underlying_out: u64) -> Result<()> {
        ...
        let execution_result = (|| -> Result<()> {
            ...
            if unwrap_fee > 0 && rift.fees_vault != anchor_lang::solana_program::system_program::ID
            {
                ...
                interface_transfer(fee_transfer_ctx, unwrap_fee)?;
            ...
            // **TOKEN-2022 MIGRATION**: Use token_interface for compatibility
            interface_transfer(transfer_ctx, amount_after_fee)?;
            ...
            if actual_received < actual_sent {
                ...
                // **FIX CRITICAL #13**: Reject transaction if destination fees exceed 1%
                require!(fee_percentage <= 1.0, ErrorCode::ExcessiveTransferFee);
                msg!("✅ Destination fee within acceptable 1% threshold");
            }
    ...
    /// **SECURITY FIX #4**: Update Pyth oracle using SDK (prevents manual byte-slice errors)
    /// Uses pyth-solana-receiver-sdk to validate account layout, status, and staleness
    pub fn update_pyth_oracle(ctx: Context<UpdatePythOracle>) -> Result<()> {
        ...
        let expected_pyth_account = rift
            .pyth_price_account
        ...
        // **SECURITY FIX #4**: Use Pyth SDK for validated price parsing
        // This replaces manual byte slicing with audited SDK that validates:
        // - Magic numbers and version
        // - Status flags (trading/halted/unknown)
        // - Staleness and publish time
        // - Price confidence intervals

        let pyth_program_id =
            Pubkey::from_str_const("FsJ3A3u2vn5cTVofAjvy6y5kwABJAqYWpe4975bi2epH");
        require!(
            ctx.accounts.pyth_price_account.owner == &pyth_program_id,
            ErrorCode::InvalidOracleOwner
        );

        // Load and validate price feed using Pyth SDK
        let price_update =
            PriceUpdateV2::try_deserialize(&mut &ctx.accounts.pyth_price_account.data.borrow()[..])
                .map_err(|_| ErrorCode::InvalidOracleData)?;

        // Get current time for validation
        let current_time = Clock::get()?.unix_timestamp;

        // Get current price with SDK validation
        // SDK automatically checks:
        // ✅ Status (trading/halted/unknown)
        // ✅ Magic numbers and account structure
        // ✅ Version compatibility
        let price_feed = price_update
            .get_price_no_older_than(
                &Clock::get()?,
                300,        // Max 5 minutes old
                &[0u8; 32], // Price feed ID (any for now)
            )
            .map_err(|_| ErrorCode::OraclePriceStale)?;

        // **FIX CRITICAL #43**: Reject future timestamps (prevents staleness bypass)
        // Even though SDK checks staleness, a future timestamp could bypass age validation
        // Attack: Future publish_time makes (current_time - future_time) negative
        require!(
            price_feed.publish_time <= current_time,
            ErrorCode::InvalidOracleData
        );

        // Extract price and confidence (SDK validates these)
        require!(price_feed.price > 0, ErrorCode::InvalidOraclePrice);

        let price = price_feed.price as u64;
        let confidence = price_feed.conf as u64;
        let exponent = price_feed.exponent;

        msg!("✅ Pyth SDK validation passed");
        msg!("   Price: {} (expo: {})", price, exponent);
        ...
        // **SECURITY FIX #50**: Validate exponent/decimals (-18 to 0 typical)
        require!(
            exponent >= -18 && exponent <= 0,
            ErrorCode::InvalidOracleExponent
        );

        // Normalize price to 6 decimals (standard for rift system)
        let normalized_price = if exponent < -6 {
            // Scale down (e.g., -8 to -6: divide by 100)
            let scale_factor = 10u64.pow((exponent.abs() - 6) as u32);
            price
                .checked_div(scale_factor)
                .ok_or(ErrorCode::MathOverflow)?
        } else if exponent > -6 {
            // Scale up (e.g., -4 to -6: multiply by 100)
            let scale_factor = 10u64.pow((6 - exponent.abs()) as u32);
            price
        ...
        // **FIX MEDIUM #44**: Validate price bounds to prevent protocol brick
        require!(normalized_price > 0, ErrorCode::InvalidOraclePrice);
        require!(
            normalized_price <= 1_000_000_000_000,
    ...
    /// Allows creator to update embedded oracle for tokens without Pyth/Switchboard feeds
    ...
    pub fn admin_update_rift_metadata(
        ...
        let metadata_exists = metadata_info.data_len() > 0;

        if metadata_exists {
            msg!("Metadata exists, updating...");
            // Use update instruction
            ...
            anchor_lang::solana_program::program::invoke(
                &update_ix.instruction(update_args),
                &[
                    ctx.accounts.metadata.to_account_info(),
            ...
            msg!("Metadata does not exist, creating...");
            // Build create metadata instruction data
            let create_ix = mpl_token_metadata::instructions::CreateMetadataAccountV3 {
                metadata: ctx.accounts.metadata.key(),
                mint: ctx.accounts.rift_mint.key(),
                mint_authority: ctx.accounts.rift_mint_authority.key(),
        ...
        msg!("Metadata created/updated successfully");
        Ok(())
    }
    ...
    pub fn set_oracle_accounts(
        ...
        msg!(
            "Oracle accounts set - Pyth: {:?}, Switchboard: {:?}",
            pyth_account,
            switchboard_account
        );
    ...
    pub fn propose_oracle_change(
        ...
        rift.pending_pyth_account = pyth_account;
        ...
        msg!("Pending Pyth: {:?}, Pending Switchboard: {:?}", pyth_account, switchboard_account);
    ...
    pub fn execute_oracle_change(ctx: Context<ExecuteOracleChange>) -> Result<()> {
        ...
        // Apply the change
        rift.pyth_price_account = rift.pending_pyth_account;
        rift.switchboard_feed_account = rift.pending_switchboard_account;

        // Clear pending state
        let executed_pyth = rift.pending_pyth_account;
        ...
        msg!(
            "Oracle accounts updated - Pyth: {:?}, Switchboard: {:?}",
            executed_pyth,
    ...
    pub fn distribute_fees_from_vault(
        ...
        if partner_amount > 0 {
            ...
            // **TOKEN-2022 MIGRATION**: Use token_interface for compatibility
            interface_transfer(partner_transfer_ctx, partner_amount)?;
        ...
        // **TOKEN-2022 MIGRATION**: Use token_interface for compatibility
        interface_transfer(treasury_transfer_ctx, treasury_amount)?;
        ...
        require!(
            actual_sent >= amount.checked_mul(99).ok_or(ErrorCode::MathOverflow)?.checked_div(100).ok_or(ErrorCode::MathOverflow)?,
    ...
    pub fn distribute_withheld_vault(
        ...
        let treasury_balance_before = ctx.accounts.treasury_account.amount;
        ...
        if partner_amount > 0 {
            if let Some(partner_account) = &mut ctx.accounts.partner_account {
                partner_account.reload()?;
                partner_received = partner_account
                    .amount
        ...
        let treasury_balance_after = ctx.accounts.treasury_account.amount;
        ...
        require!(
            total_received >= amount.checked_mul(99).ok_or(ErrorCode::MathOverflow)?.checked_div(100).ok_or(ErrorCode::MathOverflow)?,
            ErrorCode::ExcessiveTransferFee
        );
...
#[derive(Accounts)]
#[instruction(vanity_seed: [u8; 32], seed_len: u8, mint_bump: u8, partner_wallet: Option<Pubkey>, rift_name: [u8; 32], name_len: u8, transfer_fee_bps: u16)]
pub struct CreateRiftWithVanityPDA<'info> {
    ...
    /// 6 Option<Pubkey> fields = 6 bytes under-allocation with std::mem::size_of
    /// Correct size: 8 (discriminator) + 912 (struct) = 920 bytes
    #[account(
        init,
        payer = creator,
        space = 920,
    ...
    /// Metadata account to create for the rift token
...
pub struct DistributeFeesFromVault<'info> {
    ...
    #[account(
        ...
        payer = payer,
        associated_token::mint = underlying_mint,
        associated_token::authority = partner_wallet,
    )]
    pub partner_account: Option<InterfaceAccount<'info, TokenAccount>>,
...
pub struct DistributeWithheldVault<'info> {
    ...
    #[account(
        init_if_needed,
        payer = payer,
        associated_token::mint = rift_mint,
        associated_token::authority = treasury_wallet,
    )]
    pub treasury_account: InterfaceAccount<'info, TokenAccount>,
...
pub struct Rift {
    ...
    pub pending_pyth_account: Option<Pubkey>,
...
pub struct OracleChangeProposed {
    pub rift: Pubkey,
    pub pyth_account: Option<Pubkey>,
...
pub enum ErrorCode {
    ...
    InvalidOracleData,
    #[msg("Invalid oracle status - Pyth price feed not in Trading status")]
    InvalidOracleStatus,
