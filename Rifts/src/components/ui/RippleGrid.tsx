...
void main() {
    ...
    // Calculate grid intensity (this creates the lines)
    vec3 gridIntensity = vec3(0.0);
    gridIntensity += exp(-gridThickness * smoothB.x * (0.8 + 0.5 * sin(pi * iTime)));
    gridIntensity += exp(-gridThickness * smoothB.y);
    gridIntensity += 0.5 * exp(-(gridThickness / 4.0) * smoothB.x);
    gridIntensity += 0.5 * exp(-(gridThickness / 3.0) * smoothB.y);

    if (glowIntensity > 0.0) {
        gridIntensity += glowIntensity * exp(-gridThickness * 0.5 * smoothB.x);
        gridIntensity += glowIntensity * exp(-gridThickness * 0.5 * smoothB.y);
    }
    
    // Green lines on black boxes with boosted intensity
    float intensity = length(gridIntensity) * 2.0; // Boost intensity significantly
    intensity = clamp(intensity, 0.0, 1.0);
    vec3 color = vec3(intensity); // Use intensity directly for green lines
    ...
    // Ensure green lines are always visible
    vec3 finalColor = color * t * finalFade * opacity;
    float finalAlpha = max(alpha, intensity * 0.5); // Base alpha on intensity
  ...
  return (
    <div
      ref={containerRef}
      className="w-full h-full relative overflow-hidden [&_canvas]:block [&_canvas]:relative [&_canvas]:z-10"
    />
  );
};
